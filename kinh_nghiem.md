...
## 12. Bài Học Định Tuyến Chunk & Xung Đột WebGL (ChunkLoadError & Multiple Instances)
**📝 Tình trạng:** Khi đưa dự án lên GitHub Pages, trang chủ bị sập với lỗi `ChunkLoadError: Loading chunk failed (timeout)`. Con kiến 3D ở trang chủ cũng đồng thời biến mất. Sau đó, khi thử ép giải nén Webpack thủ công (`splitChunks`), lại gây ra cảnh báo `Multiple instances of Three.js being imported` làm hỏng hoàn toàn kết xuất 3D ở trang mô phỏng.
**🔍 Nguyên nhân:**
1. **Lỗi `ChunkLoadError`**: Khi thêm các thư viện 3D khổng lồ (`@react-three/fiber`, `@react-three/drei` và `three.js`) vào trang mô phỏng, Webpack mặc định trộn và nhét chúng vào các Shared Chunks (khối JS tải chung). Vì file quá nặng nên môi trường mạng chậm như GitHub Pages bị timeout, làm sập dây chuyền cả tiến trình tải trang chủ.
2. **Lỗi Xung đột thư viện (Multiple Instances)**: Cố gắng dùng `splitChunks` trong file `docusaurus.config.js` để ép tách thư viện 3D ra file riêng lại phản tác dụng rủi ro cao. Trang chủ đã dùng thư viện `<model-viewer>` (chứa một phiên bản Three.js ngầm), cộng thêm R3F dùng một bản Three.js khác. Ép gói các tệp bị trùng lập vô tình làm lộn xộn các tham chiếu nội bộ, gây sập bộ nhớ WebGL.
**💡 Giải pháp rút ra:**
- Cực kỳ thận trọng và hạn chế chỉnh sửa băm `splitChunks` thủ công trong Webpack Docusaurus nếu dự án chứa nhiều thư viện bên thứ ba đồ sộ.
- Tận dụng sức mạnh gốc của React: Chỉ cần dùng lệnh `const AntSimulation = lazy(() => import(...))` bọc với `<Suspense fallback={...}>` tại điểm neo trang (`ant-simulation.js`). Cơ chế Lazy Loading (Dynamic Import) của React sẽ tự động báo hiệu cho Webpack rạch ròi không gian code. 
- Nhờ vậy, các "cục" JS khổng lồ của 3D bị tách biệt cục bộ một cách an toàn nhất, và CHỈ bắt đầu được trình duyệt yêu cầu tải về mạch máy khi user thực sự ấn truy cập URL. Giải cứu trang chủ khỏi gánh nặng tải rác, triệt tiêu lỗi Timeout mà không làm chướng ngại logic của môi trường mô phỏng 3D!
